-- Fix: group_standings_view was ordering by (points, goals_scored) only, skipping
-- goal difference — the actual FIFA tiebreaker that ranks ahead of goals scored.
-- Same normalization logic as 20260614070205_fix_team_name_aliases.sql, ORDER BY corrected.
...
SELECT
    ...
    COALESCE(SUM(tr.goals_for), 0) - COALESCE(SUM(tr.goals_against), 0) AS goal_difference
FROM team_groups tg
LEFT JOIN team_results tr ON tr.team_name = tg.team_name
GROUP BY tg.group_name, tg.team_name
ORDER BY tg.group_name, points DESC, goal_difference DESC, goals_scored DESC;
