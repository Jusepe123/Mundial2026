...
- **FullTime scores**: cuando `match.score.duration === 'EXTRA_TIME'`, almacena `match.score.fullTime.home/away` en `home_full_score`/`away_full_score`. Necesarios para que `calculate-points` determine correctamente el ganador en partidos definidos en tiempo extra (ver calculate-points). Incluidos en la comparación de selective sync para backfill automático.
...
- **Tiempo extra**: si `home_full_score` y `away_full_score` están presentes (partido con `duration=EXTRA_TIME`), se usan para determinar el ganador y si hubo empate. El score reglamentario (`home_score`/`away_score`) sigue usándose para el caso "Exacto".
- **Sin extras ni penales**: se usa el score reglamentario para todo.
