...
| `matches` | id, external_id, home_team, away_team, home_flag, away_flag, match_date, stage, home_score, away_score, home_full_score, away_full_score, home_penalties, away_penalties, status, picks_closed, venue | UNIQUE(external_id) |
...
- **Semántica de `home_score`/`away_score`**: score final EN CANCHA — 90 minutos + tiempo extra (15+15), SIN goles de la tanda de penales (`getPitchScores()`). Para partidos con alargue es `score.fullTime`; para partidos con penales es `regularTime + extraTime` (porque `fullTime` de la API incluye los goles del shootout). Esto es lo que la app muestra y contra lo que se compara el "Exacto" de los picks.
- Trigger `calculate-points` si match cambió a finished o si el score cambió en un match ya finished
- **Selective sync**: filtra matches antes del upsert loop. Skip: already-finished (cualquier fase, grupos y knockout) cuyo score reglamentario + penales + fullTime + nombres de equipo sean idénticos a la DB; far-future (>3h) si ya están en DB. Procesa: live, transitioning, score changes en finished, upcoming (<3h), matches no existentes en DB y partidos donde DB tenga "Por definir" como equipo. El bracket propagation de knockouts finished ya no requiere re-upsertearlos cada run: lo garantiza el post-processing (lee desde DB). La comparación de scores usa `getPitchScores()` (score en cancha: 90' + alargue, sin goles de shootout), la misma lógica del upsert — si no, los partidos con penales o alargue se re-procesarían eternamente. Usa `SELECT external_id, status, home_score, away_score, home_penalties, away_penalties, home_team, away_team, home_full_score, away_full_score` inicial + `Map<external_id, MatchData>` para lookup O(1).
...
- **FullTime scores**: cuando `match.score.duration === 'EXTRA_TIME'`, almacena `match.score.fullTime.home/away` en `home_full_score`/`away_full_score`. Desde el fix del 12-jul son redundantes con `home_score`/`away_score` (que ya incluyen el alargue), pero se mantienen porque calculate-points los usa para winner/draw y la comparación del selective sync los incluye (backfill automático).
...
- **Exacto**: se compara contra `home_score`/`away_score`, que desde el 12-jul es el score final en cancha (90' + alargue, sin goles de shootout).
- **Penales**: si `home_penalties` y `away_penalties` no son null, el ganador real es quien ganó los penales. Un pick de empate también recibe winner_points (el partido terminó empatado tras 120').
- **Tiempo extra**: si `home_full_score` y `away_full_score` están presentes (partido con `duration=EXTRA_TIME`), se usan para determinar el ganador y si hubo empate (equivalen a `home_score`/`away_score` en esos partidos).
- **Sin extras ni penales**: se usa el score del partido para todo.
...
| Partidos con tiempo extra puntuaban como empate (Argentina 1-1 Cabo Verde FT 3-2, Belgium 2-2 Senegal FT 3-2) | `calculate-points` solo veía score reglamentario (1-1 / 2-2) y no detectaba ganador en tiempo extra | Nuevas columnas `home_full_score`/`away_full_score` en matches; sync-matches las almacena cuando `duration=EXTRA_TIME`; calculate-points usa fullTime para winner/draw |
| App y "Exacto" ignoraban los goles del alargue (Norway 1-1 England se veía 1-1, final real 1-2) | `home_score`/`away_score` guardaban solo el score de 90' (`getRegulationScores`) y la app + el caso Exacto leen esas columnas | `getPitchScores()`: `home_score`/`away_score` ahora guardan el score en cancha (90' + alargue, sin goles de shootout); `getPenaltyScores()` deriva penales como `fullTime - regularTime - extraTime` |
